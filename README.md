...
- **GUEST_ADMIN(포트폴리오 데모 계정) 읽기 전용 UX 미완성**: 백엔드는 GUEST_ADMIN의 생성/수정/삭제 요청을 전부 403으로 차단하므로 보안상 문제는 없지만, 허용된 3개 화면(`/game/words`, `/game/episodes`, `/game/stages`)에서 저장/삭제 버튼이 숨겨지지 않고 그대로 보입니다. 게스트가 눌러보면 에러 알림창만 뜨는 정도라 기능은 안전하나 데모 경험은 매끄럽지 않습니다. TODO: 해당 3개 뷰에서 생성/수정/삭제 버튼에 `v-if="!auth.isGuestAdmin"` 추가.
